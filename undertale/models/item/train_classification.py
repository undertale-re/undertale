...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="fine tune a pretrained undertale model for a classification task",
    )
    ...
    parser.add_argument(
        "-m",
        "--model",
        help="pre-trained model checkpoint from which to start training",
    )
    ...
    parser.add_argumnet("--classes", default=2, type=int, help="the number of classes")
    ...
    model = TransformerEncoderForSequenceClassification(
        ...
        lr=Defaults.lr,
        encoder_loc=arguments.checkpoint,
    )
