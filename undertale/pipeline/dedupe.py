...
def dedupe_by_hash(input: List[str], output: str, column: str) -> str:
    """Dedupes a dataset globally using the hash of a specified bytes column.

    Arguments:
        input: A list of paths to each chunk of the dataset.
        output: Path where the deduped dataset should be written.
        column: Name of the column whose values should be used for deduplication.
    ...
    logger.info(f"deduplicating the dataset by column: {column}")
